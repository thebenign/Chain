...
function collider.remove(obj)
    collider.list[collider.count]._collider_id = obj._collider_id
    hash:remove(obj)
    hash:updateCpl(obj)
    collider.list[obj._collider_id] = collider.list[collider.count]
    collider.list[collider.count] = nil
    ...
    for i = 1, hash.cpl.count do
        ...
        for j = 1, working_cell.n do
            for k = (j+1), working_cell.n do
                ...
                working_cell[k].colliding = false
                
                if msuv and mag then
                    if not working_cell[k].static then
                        working_cell[k].call(working_cell[j].entity, msuv)
                        working_cell[k].colliding = true
                        ...
                        working_entity.position.x = working_entity.position.x + msuv.x * (mag + 0.1)
                        working_entity.position.y = working_entity.position.y + msuv.y * (mag + 0.1)
        ...
        for y = 1, col do
            for x = 1, row do
                if x*128 > xo and x*128 < xo + 1100 and y*128 > yo and y*128 < yo+900 then
                    gb = (hash.cell[(y-1) * col + x] and hash.cell[(y-1) * col + x].count > 0) and 0 or 1
