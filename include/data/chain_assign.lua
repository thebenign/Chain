...
-- Returns a table with special assignment properties.
-- usage: table = chainAssign(arg1, arg2, ...)
-- table:one(k):does(f) where _k_ is a key and _f_ is a function
-- table.one.k will be set.
...
local chainMeta = {
  __index = function(table, key)      -- When a non-existing key is indexed:
    table[key] = {}                   -- instantiate the new index as a table
  ...
  does = function(table, value)
    assert(type(value) == "function", "Argument type mismatch in \"does\" method. Value must be a function.")
    assert(table.lhs, "\"does\" method was called before defining the left side of the assignment.")
    table[table.table_string] = value
  end,
}
